...
(function ($) {
    ...
    $(document).ready(function () {
                ...
                if (req == 1 && ((val.length == 0 && valnospace.length == 0) || (($(this).attr('type') == 'checkbox' || $(this).attr('type') == 'radio') && $(this).closest('form').find('[name="' + $(this).attr('name') + '"]:checked').length == 0))) {
                    ...
                    switch (rule) {
                        case 'email':
                            ...
                            break;
                        case 'url':
                            regExp = /^((https?|ftp):\/\/)?[a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéûşç0-9][a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéûşç0-9\.\-]{0,253}\.[a-zA-Zёа-яЁА-Я]{2,10}\/?$/gi;
                            if (val.length > 0) {
                                str = val.split('//');
                                if (str && str.length > 1) {
                                    str = str[1]
        ...
        window.tildaForm.addPaymentInfoToForm = function ($jform) {
            ...
            html += '<input type="hidden" name="tildapayment[timezoneoffset]" value="' + offsetFrom_UTC_to_Local + '" class="js-tilda-payment">';
